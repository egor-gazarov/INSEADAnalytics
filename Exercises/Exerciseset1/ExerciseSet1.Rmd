...
**Your Answers here:**
1. In dataSet1.R, in line 8 "mytickers = c("SPY", "AAPL")". Later code is being runned for all tickers asked in that line.
2. 595.5%, 0.148%, 2.39%
3. 288.7%, 0.072%, 3.02%
...
**Your Answers here:**
1.dim, nrow, ncol, str
2.MARGIN argument in 'apply' should be set to 2.
```{r}
...
**Your Answers here:**
1. Startdate is set in 'startDate = "2001-01-01"'. File does not give returns for the first day, so we have to set it to "2000-12-31"
2.
```{r}
...
mytickers = c("AMZN", "WMT", "HD")  # Other tickers for example are "GOOG", "GS", "TSLA", "FB", "MSFT", 
...
if (getdata.fromscratch){
  ...
  StockPrices1=matrix(rep(0,nrow(tmp)*length(mytickers)), ncol=length(mytickers))
  colnames(StockPrices1)<-mytickers; 
  rownames(StockPrices1)<-rownames(tmp)
  StockVolume1=StockPrices1
  StockPrices1[,1] <- tmp[,6]
  ...
  for (ticker_index in 1:length(mytickers)){
    ...
    if (!inherits(tmpdata, "try-error"))
    {
      ...
      StockPrices1[therownames,ticker_index]<-tmpdata[therownames,6] # adjusted close price
      StockVolume1[therownames,ticker_index]<-tmpdata[therownames,5] # shares volume for now - need to convert to dollars later
    } else {
      cat(ticker_to_get," NOT found")
    }
  }
  # Get the daily returns now. Use the simple percentage difference approach 
  StockReturns1= ifelse(head(StockPrices1,-1)!=0, (tail(StockPrices1,-1)-head(StockPrices1,-1))/head(StockPrices1,-1),0) # note that this removes the first day as we have no way to get the returns then!
  rownames(StockReturns1)<-tail(rownames(StockPrices1),-1) # adjust the dates by 1 day now
  
  # Now remove the first day from the other data, too
  StockPrices1 = StockPrices1[rownames(StockReturns1),]
  StockVolume1 = StockPrices1[rownames(StockReturns1),]
  colnames(StockPrices1)<-mytickers
  colnames(StockVolume1)<-mytickers
  colnames(StockReturns1)<-mytickers
  
  save(StockReturns1,StockPrices1,StockVolume1, file = "DataSet2.Rdata")
} else {
  load("DataSet2.Rdata")
}

portfolio = apply(StockReturns1,1,mean)
names(portfolio) <- rownames(StockReturns1)
pnl_plot(portfolio)
...
**Your Answers here:**
1. It counts number of events where value in myData is not equal to value in StockReturns
2. 
